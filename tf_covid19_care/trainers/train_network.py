...
def main(args):
    ...
    model = build_network([cfg.treatment_infosize, cfg.im_feedsize, cfg.patient_infosize], [cfg.severity_categories, cfg.time_range], True, name='risk_predictor')
    ...
    with summary_writer.as_default(): 
        ...
        for epoch in range(args.nepoches):
            ...
            for step in range(args.nsteps_per_epoch):
                ...
                feed = [feed_treatinfo, bt_ims, bt_patientinfo]
             
                with tf.GradientTape(persistent=False) as tape:
                    severity_cls_preds, risk_reg_preds = model(feed, training=True)                    
                    
                    severity_cls_loss = get_severity_cls_loss(severity_cls_preds, bt_severity)
                    severity_cls_losses.append(severity_cls_loss)
                    ...
                    total_loss = severity_cls_loss + 2*cured_loss + censored_loss + 5*dead_loss + rank_loss
            ...
            mean_severity_cls_loss = tf.reduce_mean(severity_cls_losses)
            ...
            for step in range(eval_sample_num//(args.batch_size)):
                ...
                severity_preds, risk_preds = model(feed, training=False)
                
                eval_severity_cls_preds.append(severity_preds)
            ...
            print ('Lr: %f \n'\
                   ...
                   ( KB.eval(optimizer._decayed_lr('float32')),
                    mean_severity_cls_loss, mean_risk_reg_cured_loss, mean_risk_reg_dead_loss, mean_risk_reg_censored_loss, mean_risk_reg_rank_loss, \
                    eval_severity_cls_loss, eval_cured_loss, eval_dead_loss, eval_censored_loss, eval_rank_loss, eval_accuracy, mean_day_distance)) 
